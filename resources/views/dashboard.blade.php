  ...
  @auth
    <div class="flex h-screen">
      ...
      <main class="flex-1 overflow-y-auto bg-gray-50">
        <!-- Remplacer la section du header -->
        ...
        <!-- Remplacer le bouton de déconnexion dans la sidebar -->
        <div class="p-4 border-t border-gray-200">
              ...
              <div class="card">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-800 mt-1">{{ $devis->count() }}</p>
                    </div>
                ...
                <div class="mt-4">
                    @if($devis->count() > 0)
                        <span class="text-sm text-yellow-600 font-medium">
                            <i class="fas fa-clock mr-1"></i> {{ $devis->where('statut', 'en_attente')->count() }} en attente
                        </span>
                    @else
                        <span class="text-sm text-gray-500">
                            <i class="fas fa-info-circle mr-1"></i> Aucun devis
                        </span>
                    @endif
                </div>
            </div>
...
<div class="card mb-8">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div class="p-4 border border-gray-200 rounded-lg">
      <div class="flex justify-between items-start">
        <div>
          <h3 class="font-medium text-gray-800">Facture Électricité</h3>
          <p class="text-sm text-gray-500">Échéance: 15/05/2025</p>
        </div>
        <span class="text-lg font-bold text-red-600">-75.000 FCFA</span>
      </div>
      <div class="mt-3 flex items-center justify-between">
        <span class="badge bg-yellow-100 text-yellow-800">Mensuel</span>
    ...
    <div class="p-4 border border-gray-200 rounded-lg">
      <div class="flex justify-between items-start">
        <div>
          <h3 class="font-medium text-gray-800">Loyer</h3>
          <p class="text-sm text-gray-500">Échéance: 01/06/2025</p>
        </div>
        <span class="text-lg font-bold text-red-600">-150.000 FCFA</span>
      </div>
      <div class="mt-3 flex items-center justify-between">
        <span class="badge bg-yellow-100 text-yellow-800">Mensuel</span>
        <button class="text-sm text-primary-600 hover:text-primary-800">
  ...
  <div class="mt-4 flex justify-between items-center">
    <p class="text-sm text-gray-500">
      ...
      Total mensuel: <span class="font-medium text-red-600">225.000 FCFA</span>
    </p>
...
<a href="{{ route('register') }}" class="sidebar-item">
        ...
        <div class="space-y-4">
            @foreach($devis as $d)
                <div class="p-4 border border-gray-200 rounded-lg">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="font-medium text-gray-800">Devis #{{ $d->id }}</h3>
                            <p class="text-sm text-gray-500">{{ $d->created_at->format('d/m/Y') }}</p>
                            <p class="text-sm text-gray-600 mt-1">{{ $d->type_projet }}</p>
                        </div>
                        <span class="badge {{ $d->statut == 'en_attente' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800' }}">
                            {{ ucfirst($d->statut) }}
                        </span>
                    </div>
                </div>
